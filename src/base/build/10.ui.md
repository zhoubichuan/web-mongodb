...
db.collection.remove(
   ...
   {
     justOne: <boolean> # justOne : （可选）如果设为 true 或 1，则只删除匹配到的多个文档中的第一个
   }
)
```
...
```sql
db.getCollection("test").find({queryWhere},{key:1,key:1})
```
...
```sql
db.getCollection("test").find({key1:value1, key2:value2})
```
